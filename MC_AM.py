...
def MC(number, seed,graphs=False):
    ...
    if graphs==True:
        ...
        # Prepare data for the bottom-left plot
        ...
        axes[0, 0].bar(bins_mass[:-1], np.histogram(M_MS, bins=bins_mass, density=True)[0] / max(np.histogram(M_MS, bins=bins_mass, density=True)[0]), 
                       width=np.diff(bins_mass), color=strong_colors["MS"], edgecolor='black', alpha=0.8, label="MS")
        axes[0, 0].bar(bins_mass[:-1], np.histogram(M_WD, bins=bins_mass, density=True)[0] / max(np.histogram(M_WD, bins=bins_mass, density=True)[0]), 
                       width=np.diff(bins_mass), color=strong_colors["WD"], edgecolor='black', alpha=0.8, label="WD")
        axes[0, 0].bar(bins_mass[:-1], np.histogram(M_NS, bins=bins_mass, density=True)[0] / max(np.histogram(M_NS, bins=bins_mass, density=True)[0]), 
                       width=np.diff(bins_mass), color=strong_colors["NS"], edgecolor='black', alpha=0.8, label="NS")
        axes[0, 0].bar(bins_mass[:-1], np.histogram(M_BH, bins=bins_mass, density=True)[0] / max(np.histogram(M_BH, bins=bins_mass, density=True)[0]), 
                       width=np.diff(bins_mass), color=strong_colors["BH"], edgecolor='black', alpha=0.8, label="BH")
        axes[0, 0].set_xlabel("Mass [Msun]")
        axes[0, 0].set_ylabel("Normalized Histogram")
        axes[0, 0].legend()

        # === Second subplot: Normalized Age Histogram ===
        axes[0, 1].bar(bins_age[:-1], np.histogram(ages_ms, bins=bins_age, density=True)[0] / max(np.histogram(ages_ms, bins=bins_age, density=True)[0]), 
                       width=np.diff(bins_age), color=strong_colors["MS"], edgecolor='black', alpha=0.8, label="MS")
        axes[0, 1].bar(bins_age[:-1], np.histogram(ages_wd, bins=bins_age, density=True)[0] / max(np.histogram(ages_wd, bins=bins_age, density=True)[0]), 
                       width=np.diff(bins_age), color=strong_colors["WD"], edgecolor='black', alpha=0.8, label="WD")
        axes[0, 1].bar(bins_age[:-1], np.histogram(ages_ns, bins=bins_age, density=True)[0] / max(np.histogram(ages_ns, bins=bins_age, density=True)[0]), 
                       width=np.diff(bins_age), color=strong_colors["NS"], edgecolor='black', alpha=0.8, label="NS")
        axes[0, 1].bar(bins_age[:-1], np.histogram(ages_bh, bins=bins_age, density=True)[0] / max(np.histogram(ages_bh, bins=bins_age, density=True)[0]), 
                       width=np.diff(bins_age), color=strong_colors["BH"], edgecolor='black', alpha=0.8, label="BH")
